		...
		if trellis.readSwitches():
			...
			for i in range(numKeys):
				...
				if trellis.justPressed(i):
					print 'v{0}'.format(i)
					if i==0:
						os.system('aplay Bass-Drum-1.wav')
					if i==1:
						os.system('aplay Ensoniq-ESQ-1-Snare.wav')
					trellis.setLED(i)
		...
		if trellis.readSwitches():
			...
			for i in range(numKeys):
				...
				if trellis.justPressed(i):
					print 'v{0}'.format(i)
					if i==0:
						thread.start_new_thread(playSound, ('Bass-Drum-1.wav',))
                                        if i==1:
						thread.start_new_thread(playSound, ('Ensoniq-ESQ-1-Snare.wav',))
					# Alternate the LED
					if trellis.isLED(i):
						trellis.clrLED(i)
					else:
